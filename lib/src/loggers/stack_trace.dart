...
class StackTraceNJ implements core.StackTrace {
  ...
  static final RegExp stackTraceRegex = RegExp(r'#[0-9]+[\s]+(.+) \(([^\s]+)\)');
  ...
  List<Stackframe> _extractFrames() {
    ...
    for (String line in lines) {
      ...
      ArgumentError.value(sourceParts.length == 4, "Stackframe source does not contain the expeted no of colons '$source'");
      ...
      Stackframe frame = Stackframe(File(sourcePath), int.parse(lineNo), int.parse(column), details);
