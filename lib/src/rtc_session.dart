...
class RTCSession extends EventManager {
  ...
  Type get causes => DartSIP_C.causes;
  ...
  void init_incoming(IncomingRequest request, [Function(RTCSession) initCallback]) async {
    ...
    _timers.userNoAnswerTimer = setTimeout(() {
      request.reply(408);
      _failed('local', null, null, null, 408, DartSIP_C.causes.NO_ANSWER, 'No Answer');
    ...
    if (expires != null) {
      _timers.expiresTimer = setTimeout(() {
        if (_status == C.STATUS_WAITING_FOR_ANSWER) {
          request.reply(487);
          _failed('system', null, null, null, 487, DartSIP_C.causes.EXPIRES, 'Timeout');
        }
      }, expires);
    }
    ...
    bool voiceonly = !sdp['media'].any((dynamic m) => m['type'] == 'video');
    ...
    if (mediaConstraints['audio'] == null) {
      mediaConstraints['audio'] = peerOffersFullAudio;
    }

    // Set video constraints based on incoming stream if not supplied.
    if (mediaConstraints['video'] == null) {
    ...
    if (mediaConstraints['audio'] != null || mediaConstraints['video'] != null) {
      ...
      try {
        ...
        request.reply(480);
        _failed('local', null, null, null, 480, DartSIP_C.causes.USER_DENIED_MEDIA_ACCESS,
  ...
  void answer(Map<String, dynamic> options) async {
    ...
    if (mediaStream != null) {
      stream = _localMediaStream = mediaStream;
    } else {
    ...
    try {
      ...
      request.reply(488);
      _failed('system', null, null, null, 488, DartSIP_C.causes.WEBRTC_ERROR,
          'SetRemoteDescription(offer) failed');
      logger.error('emit "peerconnection:setremotedescriptionfailed" [error:${error.toString()}]');
    ...
    try {
      ...
      request.reply(200, null, extraHeaders, desc.sdp, () {
        ...
        _failed(
            'system', null, null, null, 500, DartSIP_C.causes.CONNECTION_ERROR, 'Transport Error');
      });
    } catch (error, s) {
      if (_status == C.STATUS_TERMINATED) {
        return;
  ...
  void terminate([Map<String, Object> options]) {
    ...
    switch (_status) {
      ...
      case C.STATUS_1XX_RECEIVED:
        ...
        _failed('local', null, null, null, status_code, DartSIP_C.causes.CANCELED, cancel_reason);
        break;
      ...
      case C.STATUS_ANSWERED:
        ...
        _failed('local', null, null, null, status_code, DartSIP_C.causes.REJECTED, reason_phrase);
        break;
      ...
      case C.STATUS_CONFIRMED:
        ...
        reason_phrase = options['reason_phrase'] ?? DartSIP_C.REASON_PHRASE[status_code];
  ...
  void sendDTMF(String tones, [Map<String, dynamic> options]) {
    ...
    if (duration != null && !utils.isDecimal(duration)) {
      throw Exceptions.TypeError('Invalid tone duration: ${duration.toString()}');
    ...
    if (interToneGap != null && !utils.isDecimal(interToneGap)) {
      throw Exceptions.TypeError('Invalid interToneGap: ${interToneGap.toString()}');
    ...
    if (request.method == SipMethod.CANCEL) {
      ...
      */
      if (_status == C.STATUS_WAITING_FOR_ANSWER || _status == C.STATUS_ANSWERED) {
        ...
        _request.reply(487);
        _failed(
            'remote', null, request, null, 487, DartSIP_C.causes.CANCELED, request.reason_phrase);
      }
    } else {
      ...
      switch (request.method) {
        case SipMethod.ACK:
          ...
          if (_late_sdp) {
            if (request.body == null) {
              terminate(
                  <String, dynamic>{'cause': DartSIP_C.causes.MISSING_SDP, 'status_code': 400});
              break;
            }
        ...
        case SipMethod.BYE:
          if (_status == C.STATUS_CONFIRMED) {
            ...
            _ended(
                ...
                request,
                ErrorCause(
                    cause: DartSIP_C.causes.BYE, status_code: 200, reason_phrase: 'BYE Received'));
  ...
  void _close() async {
    ...
    if (_connection != null) {
      try {
        ...
        logger.error('close() | error closing the RTCPeerConnection: ${error.toString()}');
      }
    }
  ...
  FutureOr<RTCSessionDescription> _createLocalDescription(
    ...
    if (type == 'offer') {
      try {
        ...
        logger.error('emit "peerconnection:createofferfailed" [error:${error.toString()}]');
      ...
      try {
        ...
        logger.error('emit "peerconnection:createanswerfailed" [error:${error.toString()}]');
    ...
    try {
      ...
      _rtcReady = true;
      logger.error('emit "peerconnection:setlocaldescriptionfailed" [error:${error.toString()}]');
    ...
    if (early) {
      if (early_dialog != null) {
        ...
        try {
          ...
          logger.debug('$error');
          _failed('remote', message, null, null, 500, DartSIP_C.causes.INTERNAL_ERROR,
  ...
  void _receiveRefer(IncomingRequest request) {
    ...
    dynamic accept2 = (InitSuccessCallback initCallback, Map<String, dynamic> options) {
  ...
  Future<Null> _sendInitialRequest(
    ...
    if (mediaStream != null) {
      ...
      try {
        ...
        _failed('local', null, null, null, 500, DartSIP_C.causes.USER_DENIED_MEDIA_ACCESS,
            'User Denied Media Access');
        logger.error('emit "getusermediafailed" [error:${error.toString()}]');
        emit(EventGetUserMediaFailed(exception: error));
        throw error;
    ...
    try {
      ...
      _failed(
          'local', null, null, null, 500, DartSIP_C.causes.WEBRTC_ERROR, 'Can\'t create local SDP');
      if (_status == C.STATUS_TERMINATED) {
        return;
      }
  ...
  void _receiveInviteResponse(IncomingResponse response) async {
    ...
    if (utils.test100(status_code)) {
      ...
      try {
        ...
        logger
            .error('emit "peerconnection:setremotedescriptionfailed" [error:${error.toString()}]');
        emit(EventSetRemoteDescriptionFailed(exception: error));
      }
      ...
      if (response.body == null || response.body.isEmpty) {
        _acceptAndTerminate(response, 400, DartSIP_C.causes.MISSING_SDP);
        _failed('remote', null, null, response, 400, DartSIP_C.causes.BAD_MEDIA_DESCRIPTION,
            'Missing SDP');
        return;
      }
      ...
      if (_connection.signalingState == RTCSignalingState.RTCSignalingStateStable ||
        ...
        try {
          ...
          _failed('local', null, null, response, 500, DartSIP_C.causes.WEBRTC_ERROR,
      ...
      try {
        ...
        _failed('remote', null, null, response, 488, DartSIP_C.causes.BAD_MEDIA_DESCRIPTION,
            'Not Acceptable Here');
        logger
            .error('emit "peerconnection:setremotedescriptionfailed" [error:${error.toString()}]');
        emit(EventSetRemoteDescriptionFailed(exception: error));
  ...
  void _runSessionTimer() {
    ...
    // I'm not the refresher.
      ...
      _sessionTimers.timer = setTimeout(() {
        ...
        logger.error('runSessionTimer() | timer expired, terminating the session');
