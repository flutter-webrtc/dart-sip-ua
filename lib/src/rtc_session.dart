...
class RTCSession extends EventManager implements Owner {
  RTCSession(this._ua) {
    ...
    _sessionTimers = RFC4028Timers(_ua.configuration.session_timers, _ua.configuration.session_timers_refresh_method, DartSIP_C.SESSION_EXPIRES, null, false, false, null);
  ...
  final Map<int?, ReferSubscriber> _referSubscribers = <int?, ReferSubscriber>{};
  ...
  RTCDTMFSender? get dtmfSender => _senders.firstWhereOrNull((RTCRtpSender item) => item.track != null && item.track!.kind == 'audio')?.dtmfSender;
  ...
  void connect(dynamic target, [Map<String, dynamic>? options, InitSuccessCallback? initCallback]) async {
    logger.d('connect()');
    ...
    Map<String, dynamic> mediaConstraints = options['mediaConstraints'] ?? <String, dynamic>{'audio': true, 'video': true};
    ...
    if (isFromUriOptionPresent) {
      ...
      extraHeaders.add('P-Preferred-Identity: ${_ua.configuration.uri.toString()}');
    }
    ...
    _request = InitialOutgoingInviteRequest(target, _ua, requestParams, extraHeaders);
    ...
    await _sendInitialRequest(pcConfig, mediaConstraints, rtcOfferConstraints, mediaStream);
  }
  ...
  void init_incoming(IncomingRequest request, [Function(RTCSession)? initCallback]) {
    ...
    if (request.hasHeader('expires')) {
      try {
        expires = (request.getHeader('expires') is num ? request.getHeader('expires') : num.tryParse(request.getHeader('expires'))!) * 1000;
      } catch (e) {
        logger.e('Invalid Expires header value: ${request.getHeader('expires')}, error $e');
      }
    }
    ...
    _timers.userNoAnswerTimer = setTimeout(() {
      request.reply(408);
      _failed(Originator.local, null, null, null, 408, DartSIP_C.CausesType.NO_ANSWER, 'No Answer');
    ...
    if (expires != null) {
      _timers.expiresTimer = setTimeout(() {
        if (_state == RtcSessionState.waitingForAnswer) {
          request.reply(487);
          _failed(Originator.system, null, null, null, 487, DartSIP_C.CausesType.EXPIRES, 'Timeout');
        }
      }, expires);
    }
  ...
  void answer(Map<String, dynamic> options) async {
    ...
    // Audio and/or video requested, prompt getUserMedia.
      ...
      try {
        ...
        request.reply(480);
        _failed(Originator.local, null, null, null, 480, DartSIP_C.CausesType.USER_DENIED_MEDIA_ACCESS, 'User Denied Media Access');
    ...
    emit(EventSdp(originator: Originator.remote, type: SdpType.offer, sdp: processedSDP));
    ...
    try {
      if (!_late_sdp) {
        desc = await _createLocalDescription(SdpType.answer, rtcAnswerConstraints);
      } else {
        desc = await _createLocalDescription(SdpType.offer, _rtcOfferConstraints);
      }
    } catch (e) {
      request.reply(500);
    ...
    try {
      ...
      request.reply(200, null, extraHeaders, desc.sdp, () {
        ...
        _failed(Originator.system, null, null, null, 500, DartSIP_C.CausesType.CONNECTION_ERROR, 'Transport Error');
      });
    } catch (error, s) {
      if (_state == RtcSessionState.terminated) {
        return;
  ...
  void terminate([Map<String, dynamic>? options]) async {
    ...
    switch (_state) {
      ...
      case RtcSessionState.provisionalResponse:
        ...
        // Check Session Status.
        if (_state == RtcSessionState.none || _state == RtcSessionState.inviteSent) {
          _is_canceled = true;
        ...
        _failed(Originator.local, null, null, null, status_code, DartSIP_C.CausesType.CANCELED, cancel_reason);
      ...
      case RtcSessionState.answered:
        ...
        _failed(Originator.local, null, null, null, status_code, DartSIP_C.CausesType.REJECTED, reason_phrase);
        break;
      case RtcSessionState.waitingForAck:
      case RtcSessionState.confirmed:
        logger.d('terminating session');

        reason_phrase = options['reason_phrase'] as String? ?? DartSIP_C.REASON_PHRASE[status_code ?? 0];
        ...
        /* RFC 3261 section 15 (Terminating a session):
          ...
          */
        if (_state == RtcSessionState.waitingForAck && _direction == Direction.incoming && _request.server_transaction.state != TransactionState.TERMINATED) {
          ...
          _ended(Originator.local, null, ErrorCause(cause: cause as String?, status_code: status_code, reason_phrase: reason_phrase));
        }
        break;
      default:
        break;
  ...
  void sendDTMF(dynamic tones, [Map<String, dynamic>? options]) {
    ...
    // Check Session Status.
    if (_state != RtcSessionState.confirmed && _state != RtcSessionState.waitingForAck) {
    ...
    for (int i = 0; i < tones.length; i++) {
      ...
      if (tone == ',') {
        ...
        dtmfFuture = dtmfFuture.then((_) async {
          ...
          await Future<void>.delayed(Duration(milliseconds: sendInterval), () {});
        });
      }
    }
  }
  ...
  bool hold([Map<String, dynamic>? options, Function(IncomingMessage?)? done]) {
    ...
    if (_state != RtcSessionState.waitingForAck && _state != RtcSessionState.confirmed) {
      return false;
    }
  ...
  bool unhold([Map<String, dynamic>? options, Function(IncomingMessage?)? done]) {
    logger.d('unhold()');
    ...
    handlers.on(EventCallFailed(), (EventCallFailed event) {
      terminate(<String, dynamic>{'cause': DartSIP_C.CausesType.WEBRTC_ERROR, 'status_code': 500, 'reason_phrase': 'Unhold Failed'});
    });
  ...
  bool renegotiate({Map<String, dynamic>? options, bool useUpdate = false, Function(IncomingMessage?)? done}) {
    ...
    dynamic sdpSemantics = options['pcConfig']?['sdpSemantics'] ?? 'unified-plan';
    ...
    bool? upgradeToVideo;
    try {
      upgradeToVideo = (options['mediaConstraints']?['video'] != false || options['mediaConstraints']?['mandatory']?['video'] != null) && rtcOfferConstraints?['offerToReceiveVideo'] == null;
    } catch (e) {
    ...
    handlers.on(EventCallFailed(), (EventCallFailed event) {
      terminate(<String, dynamic>{'cause': DartSIP_C.CausesType.WEBRTC_ERROR, 'status_code': 500, 'reason_phrase': 'Media Renegotiation Failed'});
    });
  ...
  ReferSubscriber? refer(dynamic target, [Map<String, dynamic>? options]) {
    ...
    referSubscriber.on(EventReferRequestFailed(), (EventReferRequestFailed data) {
  ...
  void _receiveRequest(IncomingRequest request) async {
    ...
    if (request.method == SipMethod.CANCEL) {
      ...
      */
      if (_state == RtcSessionState.waitingForAnswer || _state == RtcSessionState.answered) {
        _state = RtcSessionState.canceled;
        _request.reply(487);
        _failed(Originator.remote, null, request, null, 487, DartSIP_C.CausesType.CANCELED, request.reason_phrase);
      }
    } else {
      ...
      switch (request.method) {
        case SipMethod.ACK:
          ...
          if (_late_sdp) {
            if (request.body == null) {
              terminate(<String, dynamic>{'cause': DartSIP_C.CausesType.MISSING_SDP, 'status_code': 400});
              break;
            }
            ...
            try {
              ...
              terminate(<String, dynamic>{'cause': DartSIP_C.CausesType.BAD_MEDIA_DESCRIPTION, 'status_code': 488});
              logger.e('emit "peerconnection:setremotedescriptionfailed" [error:${error.toString()}]');
              emit(EventSetRemoteDescriptionFailed(exception: error));
            }
          }
        ...
        case SipMethod.BYE:
          if (_state == RtcSessionState.confirmed) {
            ...
            _ended(Originator.remote, request, ErrorCause(cause: DartSIP_C.CausesType.BYE, status_code: 200, reason_phrase: 'BYE Received'));
            ...
            _ended(Originator.remote, request, ErrorCause(cause: DartSIP_C.CausesType.BYE, status_code: request.status_code, reason_phrase: request.reason_phrase));
          } else {
  ...
  void onTransportError() {
    ...
    if (_state != RtcSessionState.terminated) {
      terminate(<String, dynamic>{'status_code': 500, 'reason_phrase': DartSIP_C.CausesType.CONNECTION_ERROR, 'cause': DartSIP_C.CausesType.CONNECTION_ERROR});
    }
  }
  ...
  void onRequestTimeout() {
    ...
    if (_state != RtcSessionState.terminated) {
      terminate(<String, dynamic>{'status_code': 408, 'reason_phrase': DartSIP_C.CausesType.REQUEST_TIMEOUT, 'cause': DartSIP_C.CausesType.REQUEST_TIMEOUT});
    }
  }
  ...
  void onDialogError() {
    ...
    if (_state != RtcSessionState.terminated) {
      terminate(<String, dynamic>{'status_code': 500, 'reason_phrase': DartSIP_C.CausesType.DIALOG_ERROR, 'cause': DartSIP_C.CausesType.DIALOG_ERROR});
    }
  }
  ...
  Future<void> _createRTCConnection(Map<String, dynamic> pcConfig, Map<String, dynamic> rtcConstraints) async {
    ...
    _connection!.onIceConnectionState = (RTCIceConnectionState state) {
      if (_state == RtcSessionState.terminated || _state == RtcSessionState.canceled) {
        logger.d('ICE State change ignored, SIP session already terminated/canceled.');
        _iceDisconnectTimer?.cancel();
        return;
      }
      ...
      if (state == RTCIceConnectionState.RTCIceConnectionStateFailed) {
        ...
        _iceDisconnectTimer?.cancel();
        terminate(<String, dynamic>{'cause': DartSIP_C.CausesType.RTP_TIMEOUT, 'status_code': 408, 'reason_phrase': 'ICE Connection Failed'});
        ...
        if (_iceDisconnectTimer == null && !_isAttemptingIceRestart) {
          ...
          _iceDisconnectTimer = Timer(const Duration(seconds: 20), () {
            ...
            if (_connection?.iceConnectionState == RTCIceConnectionState.RTCIceConnectionStateDisconnected &&
                _state != RtcSessionState.terminated &&
                _state != RtcSessionState.canceled &&
          ...
          logger.d('ICE disconnect timer not started (already running or attempting restart).');
        }
      } else if (state == RTCIceConnectionState.RTCIceConnectionStateConnected || state == RTCIceConnectionState.RTCIceConnectionStateCompleted) {
        // If connection recovers, cancel timer and reset flag
        if (_iceDisconnectTimer != null || _isAttemptingIceRestart) {
          logger.i('ICE Connection State Connected/Completed. Canceling timer/resetting flag.');
          _iceDisconnectTimer?.cancel();
        ...
        if (_state != RtcSessionState.terminated && _state != RtcSessionState.canceled) {
          logger.w('ICE closed but SIP session state was not terminal. Terminating SIP session now.');
          terminate(<String, dynamic>{'cause': DartSIP_C.CausesType.WEBRTC_ERROR, 'status_code': 487, 'reason_phrase': 'ICE Connection Closed'});
        }
      ...
      case 'unified-plan':
        _connection!.onTrack = (RTCTrackEvent event) {
          if (event.streams.isNotEmpty) {
            emit(EventStream(session: this, originator: Originator.remote, stream: event.streams[0]));
          }
        };
        break;
      case 'plan-b':
        _connection!.onAddStream = (MediaStream stream) {
          emit(EventStream(session: this, originator: Originator.remote, stream: stream));
        };
        break;
    }
  ...
  Future<RTCSessionDescription> _createLocalDescription(SdpType type, Map<String, dynamic>? constraints) async {
    ...
    if (early) {
      if (early_dialog != null) {
        ...
        try {
          ...
          logger.d('$error');
          _failed(Originator.remote, message, null, null, 500, DartSIP_C.CausesType.INTERNAL_ERROR, 'Can\'t create Early Dialog');
          return false;
        }
      ...
      try {
        ...
        _failed(Originator.remote, message, null, null, 500, DartSIP_C.CausesType.INTERNAL_ERROR, 'Can\'t create Confirmed Dialog');
        return false;
      }
    }
  }
  ...
  void _receiveReinvite(IncomingRequest request) async {
    ...
    if (request.body == null) {
      ...
      try {
        RTCSessionDescription desc = await _createLocalDescription(SdpType.offer, _rtcOfferConstraints);
        sendAnswer(desc.sdp);
      } catch (_) {
        request.reply(500);
      }
    ...
    bool reject(dynamic options) {
      ...
      logger.i('Rejecting with status code: $status_code, reason: $reason_phrase');
    ...
    emit(EventReInvite(request: request, hasAudio: hasAudio, hasVideo: hasVideo, callback: acceptReInvite, reject: reject));
  }
  ...
  Future<RTCSessionDescription> _processInDialogSdpOffer(IncomingRequest request) async {
    ...
    if (upgradeToVideo) {
      ...
      if (hasCamera) {
        ...
        if (localStream.getVideoTracks().isEmpty) {
          logger.w('Remote wants to upgrade to video but failed to get local video');
        }
        ...
        emit(EventStream(session: this, originator: Originator.local, stream: _localMediaStream));
      } else {
        logger.w('Remote wants to upgrade to video but no camera available to send');
      }
    }
    ...
    try {
      return await _createLocalDescription(SdpType.answer, _rtcAnswerConstraints);
    } catch (_) {
      request.reply(500);
      throw Exceptions.TypeError('_createLocalDescription() failed');
    }
  ...
  void _receiveRefer(IncomingRequest request) {
    ...
    bool accept2(InitSuccessCallback? initCallback, Map<String, dynamic> options) {
      ...
      if (request.refer_to.uri.hasHeader('replaces')) {
        String replaces = utils.decodeURIComponent(request.refer_to.uri.getHeader('replaces'));
    ...
    emit(EventCallRefer(
        ...
        accept: (InitSuccessCallback initCallback, Map<String, dynamic> options) {
      ...
      case 'refer':
        {
          ...
          if (request.event!.params!['id'] != null) {
            ...
            referSubscriber = _referSubscribers[_referSubscribers.keys.toList()[0]];
          } else {
  ...
  Future<void> _sendInitialRequest(Map<String, dynamic> pcConfig, Map<String, dynamic> mediaConstraints, Map<String, dynamic> rtcOfferConstraints, MediaStream? mediaStream) async {
    ...
    if (mediaStream != null) {
      ...
      try {
        ...
        _failed(Originator.local, null, null, null, 500, DartSIP_C.CausesType.USER_DENIED_MEDIA_ACCESS, 'User Denied Media Access');
        logger.e('emit "getusermediafailed" [error:${error.toString()}]');
        emit(EventGetUserMediaFailed(exception: error));
        rethrow;
      }
    ...
    try {
      ...
      _failed(Originator.local, null, null, null, 500, DartSIP_C.CausesType.WEBRTC_ERROR, 'Can\'t create local SDP');
      if (_state == RtcSessionState.terminated) {
        return;
      }
  ...
  void _receiveInviteResponse(IncomingResponse? response) async {
    ...
    if (_state != RtcSessionState.inviteSent && _state != RtcSessionState.provisionalResponse) {
      return;
    }
    ...
    if (utils.test100(status_code)) {
      ...
      if (mediaPort == 0 && _ua.configuration.terminateOnAudioMediaPortZero) {
        _acceptAndTerminate(response, 400, DartSIP_C.CausesType.MISSING_SDP);
        _failed(Originator.remote, null, null, response, 400, DartSIP_C.CausesType.BAD_MEDIA_DESCRIPTION, 'Media port is zero');
        return;
      }
      ...
      if (_connection!.signalingState == RTCSignalingState.RTCSignalingStateStable || _connection!.signalingState == RTCSignalingState.RTCSignalingStateHaveLocalOffer) {
        try {
          ...
          _failed(Originator.local, null, null, response, 500, DartSIP_C.CausesType.WEBRTC_ERROR, 'Can\'t create offer ${error.toString()}');
        }
      }
      ...
      _failed(Originator.remote, null, null, response, response.status_code, cause, response.reason_phrase);
    }
  }
  ...
  void _sendReinvite([Map<String, dynamic>? options]) async {
    ...
    try {
      ...
      sendRequest(SipMethod.INVITE, <String, dynamic>{'extraHeaders': extraHeaders, 'body': sdp, 'eventHandlers': handlers});
    } catch (e, s) {
      ...
      switch (sdpSemantics) {
        case 'unified-plan':
          localStream.getTracks().forEach((MediaStreamTrack track) {
            if (track.kind == 'video') _connection!.addTrack(track, localStream);
            _localMediaStream?.addTrack(track);
          });
          break;
        case 'plan-b':
  ...
  void _sendUpdate([Map<String, dynamic>? options]) async {
    ...
    if (sdpOffer) {
      ...
      try {
        ...
        sendRequest(SipMethod.UPDATE, <String, dynamic>{'extraHeaders': extraHeaders, 'body': sdp, 'eventHandlers': handlers});
      } catch (error) {
        onFailed(error);
      }
    } else {
      ...
      sendRequest(SipMethod.UPDATE, <String, dynamic>{'extraHeaders': extraHeaders, 'eventHandlers': handlers});
    }
  }

  void _acceptAndTerminate(IncomingResponse? response, [int? status_code, String? reason_phrase]) async {
    logger.d('acceptAndTerminate() status_code: $status_code reason: $reason_phrase');
    ...
    if (_dialog != null || _createDialog(response, 'UAC')) {
      sendRequest(SipMethod.ACK);
      sendRequest(SipMethod.BYE, <String, dynamic>{'extraHeaders': extraHeaders});
    }
   ...
   * Uses regex on the raw SDP string so direction lines (a=sendrecv etc.) are
   * reliably set for re-INVITE hold/unhold (some servers are strict about format).
   */
  String? _mangleOffer(String? sdpInput) {
    if (sdpInput == null || sdpInput.isEmpty) return sdpInput;
    if (!_localHold && !_remoteHold) return sdpInput;

    // Match whole-line direction attributes (media-level a=sendrecv, a=sendonly, etc.)
    final directionLine = RegExp(r'^a=(sendrecv|sendonly|recvonly|inactive)\r?$', multiLine: true);

    String replaceDirection(Match m) {
      final current = m.group(1)!;
      final String replacement;
      if (_localHold && !_remoteHold) {
        replacement = current == 'sendrecv' ? 'sendonly' : (current == 'recvonly' ? 'inactive' : current);
      } else if (_localHold && _remoteHold) {
        replacement = 'inactive';
      } else if (_remoteHold) {
        replacement = current == 'sendrecv' ? 'recvonly' : (current == 'sendonly' ? 'inactive' : current);
      } else {
        replacement = current;
      }
    ...
    if (_localHold && !_remoteHold) {
      logger.d('mangleOffer() | me on hold, mangling offer (regex)');
    } else if (_localHold && _remoteHold) {
      logger.d('mangleOffer() | both on hold, mangling offer (regex)');
    } else if (_remoteHold) {
      logger.d('mangleOffer() | remote on hold, mangling offer (regex)');
    }
  ...
  void _handleSessionTimersInIncomingRequest(IncomingRequest request, List<dynamic> responseExtraHeaders) {
    ...
    if (request.session_expires != null && request.session_expires! > 0 && request.session_expires! >= DartSIP_C.MIN_SESSION_EXPIRES) {
  ...
  void _handleSessionTimersInIncomingResponse(dynamic response) {
    ...
    if (response.session_expires != null && response.session_expires != 0 && response.session_expires >= DartSIP_C.MIN_SESSION_EXPIRES) {
  ...
  void _toggleMuteAudio(bool mute) {
    if (_localMediaStream != null) {
      if (_localMediaStream!.getAudioTracks().isEmpty) {
        logger.d('No audio tracks to mute');
        return;
      }
      ...
      logger.d('Went to mute audio but local stream is null');
    }
  }

  void _toggleMuteVideo(bool mute) {
    if (_localMediaStream == null) {
      logger.d('Went to mute video but local stream is null');
      return;
    }
  ...
  void _progress(Originator originator, dynamic response, [int? status_code]) {
    ...
    emit(EventCallProgress(session: this, originator: originator, response: response, cause: errorCause));
  }
  ...
  void _accepted(Originator originator, [dynamic message]) {
    ...
    emit(EventCallAccepted(session: this, originator: originator, response: message));
  }
  ...
  void _ended(Originator originator, IncomingRequest? request, ErrorCause cause) {
    logger.d('session ended');
    _end_time = DateTime.now();
    _close();
    logger.d('emit "ended"');
    emit(EventCallEnded(session: this, originator: originator, request: request, cause: cause));
  }

  void _failed(Originator originator, dynamic message, dynamic request, dynamic response, int? status_code, String cause, String? reason_phrase) {
    logger.d('session failed');

    // Emit private '_failed' event first.
    logger.d('emit "_failed"');

    ErrorCause errorCause = ErrorCause(cause: cause, status_code: status_code, reason_phrase: reason_phrase);
    ...
    emit(EventCallFailed(session: this, originator: originator, request: request, cause: errorCause, response: response));
  }
